        ...
        if send_bundle_response.status_code == 200:
            if data["code"] == "0000":
                ...
                response1 = requests.get(
                    f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=WnlCR0RJS1J4Wnh0REVIb2dVSnM&to=0{request.user.phone}&from=GEO_AT&sms={sms_message}")
                print(response1.text)

                response2 = requests.get(
                    f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=WnlCR0RJS1J4Wnh0REVIb2dVSnM&to={phone_number}&from=GEO_AT&sms={receiver_message}")
...
def mark_as_sent(request, pk):
    if request.user.is_staff and request.user.is_superuser:
        ...
        response1 = requests.get(
            f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=WnlCR0RJS1J4Wnh0REVIb2dVSnM&to=0{request.user.phone}&from=XRAY&sms={sms_message}")
        print(response1.text)
                ...
                if transaction_channel == "ishare":
                    ...
                    if send_bundle_response != "bad response":
                        ...
                        if send_bundle_response["data"]["request_status_code"] == "200" or send_bundle_response["request_message"] == "Successful":
                            ...
                            response1 = requests.get(
                                f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=WnlCR0RJS1J4Wnh0REVIb2dVSnM&to=0{user.phone}&from=XRAY&sms={sms_message}")
                            print(response1.text)

                            response2 = requests.get(
                                f"https://sms.arkesel.com/sms/api?action=send-sms&api_key=WnlCR0RJS1J4Wnh0REVIb2dVSnM&to={phone_number}&from=XRAY&sms={receiver_message}")
                            print(response2.text)
